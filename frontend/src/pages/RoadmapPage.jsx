...
const RoadmapPage = () => {
  ...
  const create = async () => { setGenerating(true); setError(''); try { const { roadmap: generated } = await generateRoadmap(); navigate(`/roadmap/${generated._id}`); } catch (e) { setError(e.response?.data?.message || 'Unable to generate roadmap.'); } finally { setGenerating(false); } };
