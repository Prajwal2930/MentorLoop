...
const PrimaryButton = ({ children, className = '', isLoading = false, disabled = false, ...props }) => {
  ...
  return (
    <motion.button
      whileHover={!isDisabled && !prefersReducedMotion ? { y: -2, boxShadow: '0 14px 26px rgba(79, 70, 229, 0.28)' } : undefined}
      whileTap={!isDisabled && !prefersReducedMotion ? { scale: 0.98 } : undefined}
      transition={{ type: 'spring', stiffness: 420, damping: 24 }}
      className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-[0_8px_18px_-8px_rgba(79,70,229,0.72)] transition-colors hover:bg-indigo-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-55 dark:bg-indigo-500 dark:hover:bg-indigo-400 dark:focus-visible:ring-offset-slate-950 ${className}`}
      disabled={isDisabled}
      {...props}
    >
